...
import com.volcengine.speech.model.AsrParams;
...
import static com.volcengine.speech.Const.FULL_CLIENT_REQUEST;
...
//    }
    ...
    private void set_auth_header() throws NoSuchAlgorithmException, InvalidKeyException {
        if (authType == Const.AuthType.TOKEN) {
    ...
    private byte[] construct_param() throws IOException {
        ...
        header[0] = (byte) (Const.ProtocolVersion.PROTOCOL_VERSION << 4 | (header_len >> 2));
        header[1] = (byte) (FULL_CLIENT_REQUEST << 4 | Const.MessageTypeFlag.NO_SEQUENCE_NUMBER);
         header[2] = (byte) (Const.MessageSerial.JSON << 4 | Const.MessageCompress.GZIP);
        header[3] = 0;
...
//        String params_json = mapper.writeValueAsString(asr_params);
    ...
    private int parse_response(ByteBuffer msg) throws IOException {
        ...
        if (message_type == Const.MessageType.FULL_SERVER_RESPONSE) {
            ByteBuffer bb = ByteBuffer.wrap(msg_byte, payload_offset, 4);
            payload_len = bb.getInt();
            payload_offset += 4;
        } else if (message_type == Const.MessageType.SERVER_ACK) {
            ByteBuffer bb = ByteBuffer.wrap(msg_byte, payload_offset, 4);
            int seq = bb.getInt();
            payload_offset += 4;
        ...
        } else if (message_type == Const.MessageType.ERROR_MESSAGE_FROM_SERVER) {
        ...
        if (message_compress == Const.MessageCompress.GZIP) {
            payload = gzip_decompress(payload);
        }
        if (message_serial == Const.MessageSerial.JSON) {
    ...
    private byte[] construct_audio_payload(byte[] audio, boolean is_last) throws IOException {
        ...
        header[0] = (byte) (Const.ProtocolVersion.PROTOCOL_VERSION << 4 | (header_len >> 2));
        if (!is_last) {
            header[1] = (byte) (Const.MessageType.AUDIO_ONLY_CLIENT_REQUEST << 4 | Const.MessageTypeFlag.NO_SEQUENCE_NUMBER);
        } else {
            header[1] = (byte) (Const.MessageType.AUDIO_ONLY_CLIENT_REQUEST << 4 | Const.MessageTypeFlag.NEGATIVE_SEQUENCE_SERVER_ASSGIN);
        }
        header[2] = (byte) (Const.MessageSerial.JSON << 4 | Const.MessageCompress.GZIP);
        header[3] = 0;
